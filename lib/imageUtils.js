 ...
 * Server-side version using Sharp
 * @param {File|Buffer} file - The image file to process
...
export async function processProfileImage(file, options = {}) {
    const {
        ...
        quality = 0.85, // Quality 0-1 (will be converted for sharp)
        outputFormat = 'jpeg', // Consistent format
    ...
    if (typeof window === 'undefined') {
        ...
        // Convert quality from 0-1 to 1-100 for Sharp
        let sharpQuality = Math.round(quality * 100);
        sharpQuality = Math.max(1, Math.min(100, sharpQuality)); // Clamp between 1-100
        ...
        let processedBuffer = await sharp(buffer)
            ...
            .jpeg({ quality: sharpQuality, progressive: true })
            .toBuffer();

        // If file is still too large, reduce quality
        let currentQuality = sharpQuality;
        while (processedBuffer.length > maxFileSize && currentQuality > 50) {
            currentQuality -= 10;
    ...
    // Client-side processing (browser fallback - original code)
...
export function getOptimalImageFormat() {
    // Server-side: always return JPEG
    if (typeof window === 'undefined') {
        return 'image/jpeg';
    }

    // Client-side: check browser support
    const canvas = document.createElement('canvas');
