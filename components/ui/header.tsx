        ...
        <nav className="md:hidden flex items-center space-x-3">
          ...
          {userIsLoggedIn ? (
            <div className="relative">
              <button
                onClick={() => setShowUserMenu(!showUserMenu)}
                className="flex items-center space-x-1 text-gray-600 hover:text-emerald-600 transition-colors font-medium bg-white/80 rounded-full px-2 py-1.5 shadow-md hover:shadow-lg"
              >
                {user?.profilePicture ? (
                  <img
                    src={user.profilePicture}
                  ...
                  <div className="w-7 h-7 bg-emerald-500 rounded-full flex items-center justify-center text-white font-medium text-sm">
                    {(user?.name || userInitial || "U").charAt(0).toUpperCase()}
                  </div>
                )}
                <span className="hidden xs:inline text-xs font-medium">{user?.name || "Account"}</span>
              </button>
            ...
            <div className="flex items-center space-x-1">
              <Link
                href="/login"
                className="text-gray-600 hover:text-emerald-600 transition-colors font-medium text-xs px-2 py-1.5"
              >
               Sign In
              </Link>
              <Link
                href="/signup"
                className="bg-emerald-600 hover:bg-emerald-700 text-white px-2 py-1.5 rounded-lg transition-colors font-medium text-xs"
              >
                Sign Up
              </Link>
            </div>
